...
export default class HookupService {
  ...
  public async getActive(gender: string) {
    try {
      const hookup = await hookupModel
        .findOne({ status: "active", gender })
        .populate({
          path: "images.userId",
          model: "User",
  ...
  public async getAll(query: any) {
    try {
      const hookups = await hookupModel
        .find(query)
        .populate({
          path: "images.userId",
          model: "User",
  ...
  public async add(id: string, userId: string, image: string) {
    try {
      ...
      if (hookup.status == "inactive")
        throw new HttpException("hookup inactive", 404);

      if (hookup.images.length >= 20)
        throw new Error("Maximum reached..Wait till the next one");

      const userAlreadySubmitted = hookup.images.filter(
        (el: any) => el.userId == userId
      );

      console.log(userAlreadySubmitted);

      if (userAlreadySubmitted.length > 0)
        throw new HttpException(
          "You have already submitted your photo for today",
          400
        );

      const newHookup = await hookupModel.findByIdAndUpdate(
        id,
        { $push: { images: [{ userId, image }] } },
  ...
  public async updateStatusHookup(id: string, status: string) {
    try {
      const hookup = await hookupModel.findByIdAndUpdate(
        id,
        { status },
  ...
  public async getLastWinners() {
    try {
      let winnerMale = await hookupModel
        .find({ gender: "male" })
        .sort({ _id: -1 })
        .limit(1)
        .populate({ path: "images.userId", model: "User" });
      let male;
      let female;
      ...
      let winnerFemale = await hookupModel
        .find({ gender: "female" })
        .sort({ _id: -1 })
        .limit(1)
        .populate({ path: "images.userId", model: "User" });
  ...
  public async getAllWinners(gender: string) {
    try {
      const isActive = await hookupModel.findOne({ gender, status: "active" });
